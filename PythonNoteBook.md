...
2. **Content**
   ...
   1. 用逗号隔开，print函数可打印多个表达式。如果不想在两个表达式之间有空格，可以用+连接  

   2. 如果在print语句结尾加上逗号，接下来的语句会与前一条语句在同一行打印

   3. 序列解包：将多个值的序列解开，然后放到变量的序列中。例如，`x, y = 1, 2`。如果用到*，例如`a, b, *c = 1, 2, 3, 4`，那么`c = [3, 4]`

   4. 在Python中，下列的值作为布尔表达式时为假：`False  None  0  ""  ()  []  {}`，但他们本身不相等

   5. else if简写为elif  

   6. `x == y`和`x is y`的区别在于，前者检查值是否相等，后者检查是否是同一个对象（内存地址是否相同）

   7. Python中支持`0 < x < 100`

   8. ord函数：将字符转换成ASCII码，chr函数作用相反

   9. `a if b else c`若b为真返回a；否则返回c

   10. assert语句：语句是Python中用于断言条件的关键字。它用于在程序中检查某个条件是否为真，如果条件不满足，则会引发AssertionError异常。

   11. range函数：包含下限不包含上界。若希望下限为0，可只提供上限

   12. zip函数可以将两个序列压缩在一起得到一个元组的列表